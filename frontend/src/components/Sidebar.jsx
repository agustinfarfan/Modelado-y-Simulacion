...
const Sidebar = () => {
  const links = [
    { to: '/trayectoria', icon: <Rocket size={18} />, label: 'Trayectoria' },
    { to: '/impulso', icon: <Flame size={18} />, label: 'Impulso Motor' },
    { to: '/telemetria', icon: <Radio size={18} />, label: 'Telemetría GPS' },
    ...
    <aside className="w-72 bg-slate-900 border-r border-slate-800/80 flex flex-col shrink-0 relative z-10 shadow-2xl shadow-black">
      <div className="p-8 border-b border-slate-800/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-cyan-400/80 font-medium tracking-wider uppercase">Laboratorio Numérico</p>
          </div>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4 space-y-1">
        <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">Simulaciones</div>
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
      ...
      <div className="p-6 border-t border-slate-800/50 bg-slate-900/50">
        <div className="flex items-center gap-3 px-2">
          ...
          <span className="text-xs font-mono text-slate-400">Sistema en línea operativo</span>
        </div>
      </div>
    </aside>
  );
